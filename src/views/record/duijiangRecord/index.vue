  ...
  <div class="duijiangRecord">
    ...
    <div class="searchArea clearfix">
      <el-form ref="ruleForm" :model="listQuery" :rules="rules" label-width="74px" class="listQuery fr" :inline="true" size="mini" @submit.native.prevent>
        ...
        <!-- <el-form-item label="" prop="min_seconds" class="shichang">
          <el-input v-model.number="listQuery.min_seconds" class="append" type="min_seconds" placeholder="最小时长">
            <template slot="append">s</template>
          </el-input>
        </el-form-item>
        <el-form-item label="" prop="max_seconds" class="shichang">
          <el-input v-model.number="listQuery.max_seconds" class="append" type="max_seconds" placeholder="最大时长">
            <template slot="append">s</template>
          </el-input>
        </el-form-item> -->
    ...
    <div class="table noSpacingTable">
      ...
      >
        ...
        <el-table-column label="对讲号" min-width="180" align="center">
          <template slot-scope="{row}">
            {{ row.room_id || '-' }}
          </template>
        </el-table-column>
        <el-table-column label="对讲组" min-width="200" align="center">
          <template slot-scope="{row}">
            {{ row.variable_dialed_user || '-' }}
          </template>
        </el-table-column>
        <el-table-column label="对讲时间" min-width="200" align="center">
          <template slot-scope="{row}">
            {{ row.call_time || '-' }}
          </template>
        </el-table-column>
        <el-table-column label="对讲时长" min-width="200" align="center">
          <template slot-scope="{row}">
            {{ formatDuring(new Date(row.destory_timel).getTime()-new Date(row.call_time).getTime(),'s')+'s' || '-' }}
          </template>
        </el-table-column>
