...
export default {
  ...
  watch: {
    alarmListFilter: {
      handler: function(val) {
        ...
        var dataSource = new Cesium.CustomDataSource('jingqing') // 自定义一个名为dataSource1的空数据集
        this.clearJq()
        var that = this
        this.alarmListFilter.forEach((item) => {
          dataSource.entities.add({
            ...
            billboard: {
              ...
              height: 30,
              // heightReference:Cesium.HeightReference.RELATIVE_TO_GROUND,//位置相对于地形 防止实体随着相机视角漂移
        ...
        this.yuJingEntities = dataSource// 存放警情实体资源
        ...
        dataSource.clustering.clusterEvent.addEventListener(
          function(clusteredEntities, cluster) {
            ...
            // cluster.billboard.heightReference=Cesium.HeightReference.RELATIVE_TO_GROUND,//位置相对于地形 防止实体随着相机视角漂移
        ...
        //       if (ids[0].dev_type === 'jq') {
        //         that.devList = []
        //         that.devListClone = []
        //         ids.forEach((item) => {
        //           that.devList.push(item.item)
        //         })
        //         that.devListClone = cloneDeep(that.devList)
        //         that.navActive = 'selectAlarm'
        //       }
        //     } else if (ids && ids.dev_type === 'jq') {
        //       that.showDetail(ids, true)
        //     }
        //   }
  ...
  mounted() {
      ...
      if (this.jqPositionFlag) {
        ...
        entity = viewer.entities.add({
            ...
            billboard: {
              ...
              width: 30, // 如果不设置宽高 初始化无法正常显示聚合数量
              height: 30,
              heightReference:Cesium.HeightReference.CLAMP_TO_GROUND,//位置相对于地形 防止实体随着相机视角漂移
              disableDepthTestDistance: Number.POSITIVE_INFINITY,
            }
        });
    ...
    handler.setInputAction((click)=> {
        ...
        if (Cesium.defined(pickedObject) && pickedObject.id._id === 'jqPosition') {
  ...
  methods: {
    ...
    dingwei(item) {
      if (item.ALARM_LNG !== 0 && item.ALARM_LAT !== 0) {
        let entity =this.yuJingEntities.entities.getById(item.ALARM_ID)
        if(entity){
      ...
      this.$nextTick(() => {
        ...
        this.isShowJq ? this.$refs.jqDetail.style.top = `${this.viewer.canvas.clientHeight / 2 - 200}px` : ''// 保证弹窗剧中
        this.isShowJq ? this.$refs.jqDetail.style.left = `${this.viewer.canvas.clientWidth / 2 - 304}px` : ''
      })
    },
    ...
    clearJq() {
      if (this.yuJingEntities.entities && this.yuJingEntities.entities.removeAll) {
        this.yuJingEntities.entities.removeAll()
      }
    },
    // 派警按钮
    pajing(detail) {
