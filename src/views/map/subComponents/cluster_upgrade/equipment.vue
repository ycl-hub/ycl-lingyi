<template>
  <div class="equipmentWrap">
    ...
    />
    <!-- 标绘列表 -->
    <PlottingList v-if="showPlotting" ref="plottingList" :is-jing-show="isJingQingSpread" :is-biao-show="isBiaoHuiSpread" :is-spread="!isBiaoHuiSpread" :b-map="BMap" :map="map" @close="closeBiao" />
            ...
            // typeList为后台接口获取到的设备列表 mq返回的图层上图信息中的type字段应与设备列表中的设备的id对应
              if (item.is_dyna + '' === 'true') {
                if (item.AppData && item.AppData.get(String(res.POLICE_ID))) {
                  ...
                  if (item.isActive) this.getAppLocaltion(item, obj)
                }
              }
            })
          }
            ...
            this.typeList.forEach(item => {
              ...
              if (item.id === res.TYPE && item.is_dyna + '' === 'true') {
                ...
                if (oldVal) {
                  ...
                  res.yonghuzxbj=oldVal.yonghuzxbj
                }
  ...
  mounted() {
    ...
    handler.setInputAction(function(movement) {
      ...
      if (Cesium.defined(pickedLabel)) {
        ...
        // console.log(ids)
        if(primitive&&primitive.layerType === 'txtc'){
          _this.getDevInfo(primitive.overlayInfo, primitive.item, true)
        }else if(primitive&&Array.isArray(primitive._ids)){
           if (primitive._ids[0].layerType === 'txtc') { // 数据图层
            _this.$store.commit('app/SET_devlist', [])
            primitive._ids.forEach((mItem) => {
    ...
    this.AppDataTimer = setInterval(_ => {
      this.typeList.forEach((item) => {
        if (item.is_dyna === 'true' && item.AppData && String(item.GPS_STFS) !== '1' && item.primitives) { // 动态设备并且 无在线状态 按照两分钟清除一次定位
          this.viewer.scene.primitives.remove(item.primitives)
  ...
  methods: {
    getZoomLevel(cameraHeight) {
      ...
      const zoomLevels = [
        ...
        50000, 25000, 20000, 10000, 5000, 2000, 1000,
        500, 200, 100, 50, 20
      ]
    ...
    // 添加实体
    addEntities(billboardsCollectionCombine, Cesium, item, url, layerType, typeItem) {
     let billboard = billboardsCollectionCombine.add({
        id: item.GPSID || item.yonghuid,
        position: Cesium.Cartesian3.fromDegrees(item.longitude, item.latitude),
        image: url,
        width: 30, // 如果不设置宽高 初始化无法正常显示聚合数量
        height: 30,
      })
      billboard.overlayInfo=item
      billboard.dev_type=item.dev_type
      billboard.layerType=layerType
      billboard.item=typeItem
    },
    // 开启聚合
    openPoly(primitivecluster, Cesium, icon) {
      primitivecluster.enabled = true// 开启聚合
      primitivecluster.pixelRange = 50// 聚合范围
    ...
    pictureBelow(item, data, type, icon) {
      ...
      if (data && data.length > 0 && data.map) {
        if (item.primitives) {
          viewer.scene.primitives.remove(item.primitives)
          item.primitives = null
        }
        ...
        billboardsCollectionCombine._id = type;
        data.map(v => {
          if (v.list) {
            v.list.forEach(obj => {
              obj.power = item.power
              obj.is_dyna = item.is_dyna
              this.addEntities(billboardsCollectionCombine, Cesium, obj, icon, 'txtc', item)
            })
          } else {
            v.power = item.power
            v.is_dyna = item.is_dyna
            this.addEntities(billboardsCollectionCombine, Cesium, v, icon, 'txtc', item)
          }
        })
        ...
        item.primitivecluster=primitivecluster
        item.primitives=primitives
        this.openPoly(item.primitivecluster, Cesium, icon)// 开启聚合
      }
    },
    ...
    async getAppLocaltion(item, mapInfo) {
      ...
      if (item.primitivecluster && mapInfo&&item.primitivecluster._billboardCollection) {
        comLayer(mapInfo)
        let PointIndex=item.primitivecluster._billboardCollection._billboards.findIndex(item=>item.id==mapInfo.yonghuid)
        const findEntities= item.primitivecluster._billboardCollection.get(PointIndex)
        if (!findEntities&&mapInfo.yonghuzxbj) { // 不存在图层 则新增上图   否则更新点位信息即可
          this.addEntities(item.primitivecluster._billboardCollection, Cesium, mapInfo, blobIcon, 'txtc', item)
        } else {
          if (!findEntities) return
          if (findEntities&&!mapInfo.yonghuzxbj) {
            // 设备离线的情况下
            item.primitivecluster._billboardCollection.remove(findEntities)  //删除某个广告牌
          } else if(findEntities&&mapInfo.yonghuzxbj) {
            // 经纬度有变化 则更新标记对象的位置
        ...
        data.forEach((v) => {
          comLayer(v)
          this.addEntities(billboardsCollectionCombine, Cesium, v, blobIcon, 'txtc', item)
        })
        primitivecluster._billboardCollection = billboardsCollectionCombine;
        primitivecluster._initialize(scene);//初始化
        // 将标签数据添加到图元中
        ...
        item.primitivecluster=primitivecluster
        item.primitives=primitives
        this.openPoly(item.primitivecluster, Cesium, blobIcon)// 开启聚合
      }
      // 前端做警务通的聚合代码
    },
    // 移除地图marker
    markerRemove(type) {
      ...
      //   if (allOverlay[i].layerType && allOverlay[i].layerType === type) {
      //     // 移除存在的appMaker
    ...
    // 地图上点击 获取点击的点位详情
    async getDevInfo(value, item, flag) {
      const data = cloneDeep(value)
      data.is_dyna = value.is_dyna || item?.is_dyna
      data.power = value.power || item?.power
      data.id = value.id || value.UserID || value.yonghuid
      data.name = value.name || value.UserName || value.UnitName
      ...
      if (value.type === 'dev') {
        ...
        data.is_dyna = item?.is_dyna
      } else if (value.is_dyna === 'true') { // 动态设备 在线状态
        const device = this.typeList.find(item => item.id === value.dev_type || item.id === value.TYPE)
        if (device&&device.primitivecluster) {
          const PointIndex=item.primitivecluster._billboardCollection._billboards.findIndex(item=>item.id==value.id)
          const overLay= item.primitivecluster._billboardCollection.get(PointIndex)
          if (overLay) {
            data.yonghuzxbj = 1
          }
        } else {
      ...
      this.$nextTick(() => {
        ...
        if (flag) { // 从右侧已选资源跳转过来
          // 目前地图设置的最大缩放级别是18
          data.yonghuzxbj && this.viewer.camera.flyTo({
            destination: this.cesium.Cartesian3.fromDegrees(data.longitude || data.Longitude, data.latitude || data.Longitude, 28500), // 目标位置的经纬度和高度
            duration: 1,
    ...
    async getTreeDevInfo(value) {
      ...
      if ('devId' in value) {
        ...
        if (device && device.isActive && device.AppData && device.AppData.get((String(data.yonghuid)))) {
          ...
          // console.log(device, device.markerClusterer, device.markerClusterer._markers, device.markerClusterer?._markers.length)
          if (device.primitivecluster) {
            const PointIndex=device.primitivecluster._billboardCollection._billboards.findIndex(item=>item.id==value.id)
            const overLay= device.primitivecluster._billboardCollection.get(PointIndex)
            // console.log(overLay)
            if (overLay) {
              res.yonghuzxbj = 1
            }
      ...
      this.dialogTableVisible = true
      this.$nextTick(() => {
        if (data.longitude && data.latitude && data.yonghuzxbj) {
        // 目前地图设置的最大缩放级别是18
    ...
    searchGuiji: throttle(function() {
      this.$api.common.loadGpsHistory({
        ...
        data.forEach((val, key) => {
          arr.push(new this.BMap.Point(val.Longitude, val.Latitude))
        })
        // 起点
        var ptS = new this.BMap.Point(first.Longitude, first.Latitude)
        var myIconS = new this.BMap.Icon(start, new this.BMap.Size(25, 25), {})
        myIconS.setImageSize(new this.BMap.Size(25, 25))
        var markerS = new this.BMap.Marker(ptS, { icon: myIconS })
        markerS.layerType = 'guiji'
        this.map.addOverlay(markerS)
        // 折线
        var polyline = new this.BMap.Polyline(arr, { strokeColor: '#4236FF', strokeWeight: 2 }) // 创建折线
        polyline.layerType = 'guiji'
        this.map.addOverlay(polyline) // 增加折线
        // 终点
        var ptE = new this.BMap.Point(last.Longitude, last.Latitude)
        var myIconE = new this.BMap.Icon(end, new this.BMap.Size(25, 25), {})
        myIconE.setImageSize(new this.BMap.Size(25, 25))
        var markerE = new this.BMap.Marker(ptE, { icon: myIconE })
        markerE.layerType = 'guiji'
    ...
    drawCircle() {
      ...
      this.handler.setInputAction(()=> {
        ...
        let entities = []
        this.typeList.forEach((item)=>{
          if(item.primitivecluster&&item.primitivecluster._billboardCollection) {
            entities.push(item.primitivecluster._billboardCollection._billboards)
          }
        })
        entities.flat().forEach((item) => {
          var str = Cesium.Cartesian3.distance(center, Cesium.Cartesian3.fromDegrees(item.overlayInfo.longitude, item.overlayInfo.latitude))
          if (str <= radius) {
    ...
    polygon() {
      ...
      this.handler.setInputAction((click) => {
        ...
        entities.flat().forEach((item) => {
          const po = Cesium.Cartesian3.fromDegrees(item.overlayInfo.longitude, item.overlayInfo.latitude)
