      ...
      <div class="pj_con_c">
        <!-- <el-tabs v-model="activeTab">
          <el-tab-pane v-for="type in typeList" :key="type.id" :label="type.name" :name="type.id" lazy>
            <components :is="type.comp" :b-map="BMap" :map="map" :detail="detail" />
          </el-tab-pane>
        </el-tabs> -->
        <JIANKONG ref="JIANKONG" :b-map="BMap" :map="map" :detail="detail" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 关闭派警弹窗
    close() {
      if (this.$refs['JIANKONG'].videoGroupCallDialog) {
        this.$message({
