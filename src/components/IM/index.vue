              ...
              <ul v-if="activeTab==='single'">
                <li v-for="user in userList" :key="user.yonghuid" class="listItem clearfix" :style="{background:user.yonghuid===activeUserOrGroup.yonghuid?'#005580': ''}" @click="listClick(user)">
                  ...
                  <div class="itemRight clearfix">
                    ...
                    <p class="message" :title="user.UnitName">{{ user.UnitName }}</p>
                  </div>
                </li>
          ...
          <div class="right">
            <div v-if="activeTab==='message'&&activeListItem" class="rightContentHis">
              <div class="titleAndHis">
                ...
                <div class="chat" :class="{'chatGroup': activeListItem.TYPE==='1'}">
                        ...
                        <div v-else class="singleChatContainer2">
                          ...
                          <div class="chatRight clearfix">
                            <div class="nameTime">
                              ...
                              <span v-if="msg.TYPE==='0'" class="time">{{ msg.IS_READ==1?'已读':'未读' }}</span>
                            </div>
                            <div v-if="msg.MSGTYPE==='0'" class="chatCon">
                              {{ msg.CONTENT }}
                            </div>
            ...
            <!-- 成员管理抽屉 -->
            <GroupMember v-if="drawer" :drawer.sync="drawer" :active-user-or-group="activeUserOrGroup" @getGroup="getGroupList" />
            <!-- 修改群组名称 -->
            <ReGroupName v-if="reGroupNameDialog" :re-group-name-dialog.sync="reGroupNameDialog" :active-user-or-group="activeUserOrGroup" @GroupName="GroupName" />
          </div>
        </div>
      </div>
      <el-dialog
...
import single from '@/assets/images/im/single.png'
import single_h from '@/assets/images/im/single_h.png'
...
export default {
  ...
  computed: {
    isShow: {
      ...
      set(val) {
        this.$emit('update:messageDialog', val)
      }
    },
    ...mapGetters([
      'userAgent',
    ...
    mqResponseData: {
      handler: function(val) {
        ...
        if (json.MSG_KEY !== 'REPORT_LOCATION') {
          console.log(val, 'mqResponseData')
        }
        // console.log(json)
        // 创建群组或者新增人员到群组
        if (json.MSG_KEY === 'CreatGroup' && !json.MEMBERLIST && json.TEMP === '0') {
        ...
        // 删除群组内的人员
        if (json.MSG_KEY === 'DelMember' && json.TEMP === '0') {
        ...
        // 更改群组名称
        if (json.MSG_KEY === 'RENAME_GROUP' && json.TEMP === '0') {
        ...
        // 解散群组给所有人推送，包括自己
        if (json.MSG_KEY === 'DelGroup' && json.TEMP === '0') {
          // 重新获取新的群组
          this.getGroupListToTop()
          // 删除存在记录中的群组
          for (let index = 0; index < this.imList.length; index++) {
        ...
        if (json.MSG_KEY === 'ReceiveMessage') {
          ...
          this.flag = true
          // 首先判断list列表里面有
          if (json.SENDER !== this.userInfo.userName && !this.messageDialog && (this.activeTab !== 'message' && this.messageDialog)) {
            // 只有不在当前IM实时通信界面 并且不在聊天记录界面时 弹出提示信息框
                ...
                if (item.id === json.SENDER) {
                  ...
                  if (!flag) {
                    ...
                    if (this.imListActiveId !== json.SENDER) {
                      ...
                      // 处理选中状态时候的消息推送
                      this.client.send('/exchange/SERVER_IM_BACK_MSG/SERVER_IM_BACK_MSG', {}, `{
                        "MSG_KEY": "MSG_READ",
                        "MSG_ID": "${json.MSG_ID}",
                        "SENDER_ID": "${json.SENDER}",
                        "RECEIVER_ID": "${json.RECEIVER}"
                      }`)
                    }
                  }
                }
        ...
        // 阅读消息处理
        if (json.MSG_KEY === 'MSG_READ' && json.MSG_ID) {
          // console.log(json, '已读消息')
          if (this.list.length > 0) {
            for (let index = 0; index < this.list.length; index++) {
              var item = this.list[index]
              if (item.TYPE === '0') {
                for (let i = 0; i < item.MSG_LIST.length; i++) {
                  // console.log(item.MSG_LIST[i])
                  if (item.MSG_LIST[i].MSG_ID === json.MSG_ID) {
                    this.$set(item.MSG_LIST[i], 'IS_READ', 1)
  ...
  methods: {
    GroupName(obj) {
      // 修改群组名称 页面实时更新
      const keyIndex = this.groupList.findIndex(item => item.GROUP_ID === obj.GROUP_ID)
      // console.log(obj, keyIndex)
      this.$set(this.groupList[keyIndex], 'GROUP_NAME', obj.GROUP_NAME)
    },
    ...
    async handleScroll() {
      ...
      if (this.$refs.scrollList.scrollTop === 0) {
        ...
        const res = await this.$api.im.GetChatMsgHis({
          ...
          Top_N: this.activeListItem.loadNumber * 5,
    ...
    getGroupList() {
      return this.$api.im.GetGroupList({
        TOKEN: this.TONE_voip.cmd.TOKEN,
        PoliceNo: this.TONE_voip.cmd.POLICE_ID,
        TEMP: '0'
      }).then(res => {
    ...
    listClick(item) {
      ...
      if (item.unRead) {
        ...
        // 单聊处理已读未读
        if (item.TYPE === '0') {
          const unReadList = item.MSG_LIST.slice(-item.unRead)
    ...
    sendMessage: throttle(function() {
      // console.log(this.Content.replace(/(^s*)|(s*$)/g, '').length, this.isNull(this.Content))
      if (this.Content.replace(/(^s*)|(s*$)/g, '').length === 0 || this.isNull(this.Content)) {
        this.$message.closeAll()
        this.$message.warning('不能发送空白信息！')
        return false
      }
      const MSG_ID = this.guid.newGUID()
    ...
    beforeAvatarUploadFile(file) {
      ...
      if (!isPdf && !isZip && !isDoc && !isPpt && !isRar && !isDocx && !isXls && !isXlsx) {
        this.$message.warning('文件只能是 pdf、zip、doc、docx、xls、xlsx、ppt、rar格式!')
        return false
      }
      if (!isLt8M) {
        this.$message.warning('文件大小不能超过 30MB!')
        return false
      }
