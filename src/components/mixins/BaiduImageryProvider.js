...
// const Cesium = require('cesium/Cesium.js')// 引入Cesium
...
BaiduImageryProvider.prototype.requestImage = function(x, y, level) {
  ...
  var s = this.url.replace('{x}', x - r / 2).replace('{y}', c / 2 - y - 1).replace('{z}', level).replace('{s}', Math.floor(10 * Math.random())).replace('{udt}', new Date().getTime())
